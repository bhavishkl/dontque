...
export default function ManageDefault({ params, queueData: initialQueueData, isLoading: initialLoading }) {
  ...
  const handleUpdateServiceTime = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to update service time')
  ...
  const handleServed = async (entryId) => {
    ...
    try {
      const response = await fetch(`/api/queues/${params.queueId}/customers/${entryId}/serve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ queueData: queueData.queueData, customersInQueue })
      })
      ...
      setCustomersInQueue(prev => prev.filter(customer => customer.entry_id !== entryId))
    } catch (error) {
  ...
  const handleNoShow = async (entryId) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to mark customer as no-show')
